...
def flip_img(filename):
    ...
    image_crop = tf.random_crop(image_float, [scale_height, scale_width, 3])
...
def main(_):
    with tf.Session(config=config) as sess:
        ...
        fake_X = generator(Z, regularizer, is_training=is_training, reuse=False)
        fake_logits = discriminator(fake_X, regularizer, is_training=is_training, reuse=False)
